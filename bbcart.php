...
// Check if the page is the cart page before running queries
// if ($_SERVER['REQUEST_URI'] == 'bbcart.php') {
    ...
    // Execute the query to count the cart items
    $result2 = $con->query($query2);

    if ($result2) {
        // Fetch all rows and count them
        $total_items = $result2->fetch_row()[0]; 
         // num_rows will give the number of rows returned by the query
    } else {
...
//         $currentDate = date('Y-m-d'); // Format: Year-Month-Day (e.g., 2024-11-29)

//         // SQL query to insert the order
//         $sql = "INSERT INTO projtest.orders (user_id, order_date, bill, address) 
//         VALUES ('$user_id', '$currentDate', '$total', '$Address')";

//         // Execute the query and check if successful
//         if ($con->query($sql) === true) {
//             // Retrieve the order_id of the just inserted order
...
//                     $sql_order_item = "INSERT INTO projtest.order_item (product_id, order_id, quantity, price)
//                     VALUES ('$product_id', '$order_id', '$quantity', '$price')";

//                     if ($con->query($sql_order_item) === TRUE) {
//                         // echo "Item added to order: Product ID: $product_id, Quantity: $quantity, Price: $price <br>";
//                     } else {
//                         echo "Error: " . $con->error;
//                     }
//                 }

//                 // Optionally, you can empty the cart after placing the order
...
// Output the contents if there are items in the cart
 }else {
